...
if (($MemberSub['receive_messages'] != 1 && $subscription['profile_type']!="claim") || ($subscription['profile_type']=="claim" && isset($addOn['status']) && $addOn['status'] === 'success') ) { ?>
	...
	<div class="well <?php if ($subscription['profile_connection_banner_position'] != "above_profile_photo"){ ?> tmargin <?php } ?> bmargin xs-text-center make-connection" style="padding:10px 15px; margin-left: 30px; margin-right: 30px;">
