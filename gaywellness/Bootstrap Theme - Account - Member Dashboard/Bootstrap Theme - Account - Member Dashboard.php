<?php
// Free Membership Level Mover (luisalvarado@ubuntu.com)

// To which Membership Level IDs the Free Trial applies to
$freeTrialArray = array(78);

// Membership Level ID to which the user will be moved to once the member reaches X amount of days.
$dropToMembershipLevelID = 77;

// Total amount of Days the Member should enjoy the Free Trial before the account gets moved to another membership level
$totalDays = 1;

if (in_array($me['subscription_id'], $freeTrialArray)) {
// Transforms multiple DATE formats to a single one. Then processess this to show the amount of Days
$datediff = time() - strtotime(date("Y-m-d", strtotime($me[signup_date])));

// Calculates how many days have passed since the Member Signed Up
$daysPassed = round($datediff / (60 * 60 * 24));

if ($daysPassed >= $totalDays) {
echo 'Your FREE TRIAL is now over. You enjoyed '.$totalDays.' day of the Massage Therapy - RAINBOW+Boost plan but have now been downgraded to your original Massage Therapy - RAINBOW PLAN.  <u><a href="/digital-products/boost-to-featured-massage" class="btn btn-primary btn-block">Purchase another boost!</a></u>';
$endFreeMonths = mysql($w[database], 'UPDATE users_data SET subscription_id = "'.$dropToMembershipLevelID.'" WHERE user_id = "'.$me['user_id'].'"');
} else {
echo "<center>You have ".($totalDays - $daysPassed).' Day(s) Left in your RAINBOW+Boost Plan.  </center>';
...
?> <?php
    ...
    if ($subscription['show_dashboard_manage_listing'] != "0") {
        ?>
        <div class="col-md-4 dashboard-module dashboard-manage-listing">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h4 class="nomargin bold">
...
<?php if ($user[subscription_id] == '5') { ?>
			[widget=Bootstrap Theme - Search Module - Sub_Location] <?php } ?>
        </div>
    <? };
