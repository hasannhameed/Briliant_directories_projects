...
<div class="col-xs-6 col-sm-3 col-md-3 nolpad member_sidebar">
    <div class="well member_admin_sidemenu" style="padding-top:10px;">
        ...
        if($user_data['parent_id'] > 0){
            ...
            ?>
            <?php if ($_COOKIE['parentToken'] != "" && $user['parent_id'] != 0){ ?>
        ...
        <?php if ($user_data['full_name'] != "") { ?>
            <div style="text-transform: capitalize;word-wrap: break-word;" class="bold text-center">
                <?php echo $user_data['full_name']; ?>
            </div>
		<?php } ?>
		<?php if ($userPhoto != "") { ?>
            <a href="/account/profile">
                <img src="<?php echo $userPhoto ?>" style="max-height:160px;" alt="Member Profile Image" class="img-thumbnail center-block bmargin">
            </a>
        ...
        if ($user_data['filename']!="" && $subscription['searchable'] == "1") { ?>
            <a target="_blank" href="/<?php echo $user_data['filename']?>" style="" class="btn btn-primary btn-sm bold btn-block no-radius-bottom view-listing-link">
                %%%view_public_listing_account%%%
                <i class="fa fa-external-link fa-fw"></i>
            </a>
        <?php } ?>
        <span class="module center-block fpad member-account-information text-center bmargin <?php if ($user_data['filename']!="" && $subscription['searchable'] == "1") { ?>no-radius-top<?php } ?>" style="padding-top:5px;">
         
            <span class="small member-level-name">
                
            </span>
            <?php if (upgradeAvailable($user_data,$w)>0 && ($user['parent_id'] == "0" || $user['parent_id'] == "")) { ?>
                <a href="/account/upgrade" class="btn btn-tarracotta btn-sm bold btn-block upgrade-listing-link" style="margin-top: 5px;">
                %%%upgrade_listing%%%
            </a>
            <?php } ?>
        </span>
        ...
        <div class="panel-group sidemenu_panel" id="accordion" role="tablist" aria-multiselectable="true">
            ...
            <div class="panel panel-default">
                ...
                <div id="collapseThree" class="panel-collapse collapse <?php echo $listingVarState; ?>" role="tabpanel" aria-labelledby="headingThree">
                    <div>
                        <a href="/account/contact" class="list-group-item <?php if($pars[1] == "contact"){?>bold<?php }?>">%%%dash_label%%% %%%account_listing_tab_1%%%</a>
                        <?php
                        if ($sub['show_profile_photo'] == 1 || $sub['show_logo_upload'] == 1) { ?>
                            <a href="/account/profile" class="list-group-item <?php if($pars[1] == "profile"){?>bold<?php }?>">%%%dash_label%%% %%%account_listing_tab_2%%%</a>
                        <?php }
                        if ($sub['show_listing_details'] == 1) { ?>
                            <a href="/account/resume" class="list-group-item <?php if($pars[1] == "resume"){?>bold<?php }?>">%%%dash_label%%% %%%account_listing_tab_3%%%</a>
                        <?php }
                        if ($sub['show_about_tab'] == 1) { ?>
                            <a href="/account/about" class="list-group-item <?php if($pars[1] == "about"){?>bold<?php }?>">%%%dash_label%%% %%%account_listing_tab_4%%%</a>
                        <?php }
                        ...
                        if ((isset($multiListing2['status']) && $multiListing2['status'] === 'success') && $sub['location_limit'] > 0 && $sub['location_limit'] != '') { ?>
                            <a href="/account/locations" class="list-group-item">%%%dash_label%%% %%%account_listing_tab_5%%%</a>
                        <?php }
                        if ($w['promote_feature'] == 1) {

                            if ($w['promote_feature_verify'] == 1 && $user_data['verified'] == 0) { ?>
                                <a href="/account/promote/verify" class="list-group-item">%%%dash_label%%% %%%verify_listing%%%</a>
                                <?php
                            }
            ...
            if (isset($addonStatistics['status']) && $addonStatistics['status'] === 'success' && $sub['profile_statistics'] == "1") { ?>
                <div class="panel panel-default">
                    <div class="panel-heading" id="headingOne">
                        <div class="panel-title">
							<a href="/account/stats/2024/11" class="sidebar-profile-stats-link">
                ...
                <div id="collapseFour" class="panel-collapse collapse<?php echo $settingVarStat; ?>" role="tabpanel" aria-labelledby="headingFour">
                    <div>
                        <?php if($sub['hide_billing_links'] != 1){?>
                            <a href="/account/billing" class="list-group-item <?php if($pars[1] == 'billing'){ ?>bold<?php } ?>">%%%dash_label%%% %%%billing_details_account_sidebar%%%</a>
                        <?php }?>
                        <a href="/account/changelisting" class="list-group-item <?php if($pars[1] == 'changelisting'){ ?>bold<?php } ?>">%%%dash_label%%% %%%manage_account%%%</a>
                        <a href="/account/password" class="list-group-item change-password-sidebar <?php if($pars[1] == 'password'){ ?>bold<?php } ?>">%%%dash_label%%% %%%profile_edit_pw%%%</a>
