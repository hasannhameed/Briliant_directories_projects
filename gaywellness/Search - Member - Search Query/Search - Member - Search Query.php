...
class BDMembersQuery extends queryBuilder
{
    ...
    public function runSubLevelCategory($returnQuery){
        ...
        if ( ( (!empty($this->subLevelId) ) || !empty($this->subSubLevelId) ) && $this->serviceId <= 0 ) {
            ...
            if (is_array($mainLevelId) && !empty($mainLevelId)) {

                // SECURITY: Ensure IDs are numbers to prevent errors/attacks
                $safeIds = implode(",", array_map('intval', $mainLevelId));
                $requiredCount = count($mainLevelId);

                // 2. BASIC FILTER (The "Net")
                // Find any row that matches ONE of the IDs. 
                // We removed "OR st.service_limit = 'all'" because Strict Search requires specific matches.
                $whereParameter = " (rs.rel_service_id IN ($safeIds)) ";

                // 3. STRICT LOGIC (The "Filter")
                // If user selected multiple categories, we force them to match ALL of them.
                // We use addQueryParameter so the framework puts these in the right place (at the end).
                if ($requiredCount > 1) {
                    $this->addQueryParameter('groupByParameters', 'ud.user_id');
                    $this->addQueryParameter('havingParameters', "COUNT(DISTINCT rs.rel_service_id) >= $requiredCount");
                }

                // 4. PROFESSION LOGIC
                // We append this with "AND" to ensure the profession matches strictly as well.
                if ($this->target === 'search' && empty($this->searchQuery) && !is_null($this->topLevelId)) {
                    $topIds = implode(",", $this->topLevelId);
                    $whereParameter .= " AND (list_service_profession_id = ud.profession_id OR ud.profession_id IN ($topIds)) ";
                } else {
                    // Default profession check
                    $whereParameter .= " AND (list_service_profession_id = ud.profession_id) ";
                }
                ...
                // 6. SELECT COLUMNS
                $relServiceSTblSelect = " rss.service_id IN ($safeIds) ";
            }
