...
<script>
function onSubmitcode(fid,e){
e.preventDefault();
var form_name = '#' +fid;
    $.ajax({
         type: "POST",
		 data : $(form_name).serialize(),   
         url:  "/api/widget/html/get/landpage_edit ",		 
			 success: function(data){
				 if(data=="failuresuccess"){
					  swal({
					title: "Success",
...
var form_name = '#' +fid;
    $.ajax({
         type: "POST",
		 data : $(form_name).serialize(),   
         url:  "/api/widget/html/get/img_path_save ",		 
			 success: function(data){
				 }
...
<? for($i=1;$i<=25;$i++){ ?>	 
    ...
    $.ajax({
        url: '/api/widget/html/get/imgupload', 
		 type: 'POST', 
        data: formData,                        
		cache: false,
        contentType: false,
...
<script type='text/javascript' src='/landingpage/jss/layers.frameworks.js'></script>
<script type='text/javascript' src='/landinvgpage/js/plugin.js'></script>
<? 
$dbt=brilliantDirectories::getDatabaseConfiguration('database');
 $pageld=mysql(brilliantDirectories::getDatabaseConfiguration('database'),"SELECT * FROM `landing_page_fields` WHERE id=39"); 
$pagesd=mysql_fetch_assoc($pageld);
...
//$('#OpenImgUpload').click(function(){ $('#imgupload').trigger('click'); });
  ...
  $('.editable').on('dblclick', function() {
	...
	var inputs=$("#wrapper-content").find('textarea').length;
if(inputs==1){	
var edata='"'+  dataname + '"';
      data = $("<textarea class='edib froala-editor' name='"+ dataname +"'></textarea><a href='javascript:void(0);' onclick='editData("+ edata +");' style='border-radius: 8px;background-color:#fff;color:#000;padding:8px;font-size: 20px;text-decoration:none;font-weight:bold;'><i class='fa fa-check-circle' style='font-size:23px;color:green;padding-left: 3px;padding-right: 3px;'></i>SAVE</a>").prop({
        'type': 'text',
        'value': value.html()
      });
         ...
         $(document).ready(function() {
            ...
            $('.froala-editor').froalaEditor({
               placeholderText: '<?php echo $label['froala_type_something']?>',
                ...
                maxHeight: '300',
                toolbarButtons: ['fullscreen', 'bold', 'italic', 'underline', 'strikeThrough', 'subscript', 'superscript', 'fontFamily', 'fontSize', '|', 'color', 'emoticons', 'inlineStyle', 'paragraphStyle', '|', 'paragraphFormat', 'align', 'formatOL', 'formatUL', 'outdent', 'indent', 'quote', 'insertHR', '-', 'insertLink', 'insertTable', 'undo', 'redo', 'clearFormatting', 'selectAll','embedly', 'html'],
            });           
        });	  
  });
     }); 
...
if($imgdata22['status']==1){
$imagepath22="uploads/".$imgdata22[file];
}else{
$imagepath22="https://bestwpdevelopers.com/uploads/".$imgdata22[file];
}?>
<style>
 .layers-widget-column-134-186 { background-repeat: no-repeat;background-position: center;background-image: url('<?=$imagepath22;?>');}
</style>