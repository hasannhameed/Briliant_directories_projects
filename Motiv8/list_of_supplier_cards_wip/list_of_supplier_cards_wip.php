...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_attended'])) {
    ...
    if (mysql_query($delete_sql)) {
        activity_log('Delete', 'attending_staff_attendance', $post_id, 'Staff Attendance Deleted by ' . $user_full_name . ' for Supplier ID: ' . $supplier_id . ' and Staff ID: ' . $staff_id . ' for Post ID: ' . $post_id);
        $updateSql = "UPDATE supplier_registration_form 
                      SET limit_reached = 0 
                      WHERE user_id = $supplier_id AND event_id = $post_id";
        activity_log('Update', 'supplier_registration_form', $post_id, 'Supplier Registration Form Updated by ' . $user_full_name . ' for Supplier ID: ' . $supplier_id . ' and Event ID: ' . $post_id . ' to limit_reached: 0');
...
function formatDeadlineCounter($post_live_date, $fallback_date = null)
{
    ...
    if ($date < $now) {
        $result['text']     = 'Deadline has passed';
        $result['disabled'] = true;
        return $result;
    }

    if ($date == $now) {
        $result['text'] = 'Deadline is today';
        return $result;
    }
...
$deadlineInfo = formatDeadlineCounter($deadline, $start_date);
                                        ...
                                        <div class="panel-heading">
                                            <h4 class="panel-title"
                                                ...
                                                <span class="text-danger lpad" style="flex-grow: 1; text-align: right;">
                                                    <?php $deadlineInfo = formatDeadlineCounter($deadline, $start_date); ?>
                                                    Deadline:  <?= htmlspecialchars($deadlineInfo['text']) ?>
                                                </span>
                                                            ...
                                                            while ($staff = mysql_fetch_assoc($attendingStaff)) {
                                                                ...
                                                                if ($user_data['user_id'] == $staff['supplier_id']) {
                                                                    ...
                                                                    $CoordinatorLabel = ($isCoordinatorExists)
                                                                        ? '<div style="text-align: center;"><strong>Event Coordinator</strong><a href="/login/token/' . $tokensql['token'] . '/add-supplier-card/view/'.$logback_admin['token'].'" class="btn btn-primary">Login as Member</a></div>'
                                                                        : '<a href="/login/token/' . $tokensql['token'] . '/add-supplier-card/view/'.$logback_admin['token'].'" class="btn btn-primary">Login as Member</a>';
                                                                    }
                                                                } else {
                                                                    $showDataId = ($staff['user_id'] == $user_data['user_id']) 
                                                                        ? '<a href="/login/token/' . $logback_admin['token'] . '/add-supplier-card/view" class="btn btn-primary back-to-company-from" data-id="' . $staff['user_id'] . ' ' . $user_data['user_id'] . '">Back to company</a>'
                                                                        : '';
                                                                
                                                                    $CoordinatorLabel = ($isCoordinatorExists)
                                                                        ? '<center><b>Event Coordinator</b>' . $showDataId . '</center>'
                                                                        : '<center>' . $showDataId . '</center>';
                                                                }
...
$disabled = $deadlineInfo['disabled'] ? 'disabled' : '';
$style    = $deadlineInfo['disabled'] ? ' disabled-select' : '';

echo '<select class="form-control attending_staff' . $style . '" 
        name="attending_staff[]" 
        data-id="' . htmlspecialchars($staff['id']) . '" ' . $disabled . '>
        <option value="">Select Attending Staff</option>';
                                                                ...
                                                                if (($staff['supplier_id'] == $user_data['user_id'] || $staff['staff_id'] == $user_data['user_id']) && $staff['status'] == 'TBC') {
                                                                        echo '<br>
                                                                ...
                                                                if ($staff['status'] == 'TBC') {
                                                                    ...
                                                                    if (!$notRequired) {
                                                                        ...
                                                                        if ($user_data['user_id'] == $staff['supplier_id']) {
                                                                            ...
                                                                            if(!$isEmailUsed) {
                                                                                echo "";
                                                                                // echo ' <div style="padding-top:10px;"> <a href="/login/token/' . urlencode($tokensql['token']) . '/add-supplier-card/view/' . urlencode($logback_admin['token']) . '" class="btn btn-primary">Login as Member</a> </div> ';
                                                                            }
                                                                            }
                                                                        }
                                                            ...
                                                            while ($staff = mysql_fetch_assoc($attendingStaff)) {
                                                                ...
                                                                if ($user_data['user_id'] == $staff['supplier_id']) {
                                                                    ...
                                                                    // if ($pars['3'] == $logback_admin['token']) {
                                                                        $CoordinatorLabel = ($isCoordinatorExists) ? '<center><b>Event Coordinator</b><a href="/login/token/' . $logback_admin['token'] . '/add-supplier-card/view" class="btn btn-primary back-to-company-from">Back to company</a></center>' : '<center><a href="/login/token/' . $logback_admin['token'] . '/add-supplier-card/view" class="btn btn-primary back-to-company-from">Back to company</a></center>';                                                                                  
                                                                    // } else {
                                                                    ...
                                                                    if (!$notRequired) {
                                                                        ...
                                                                        if ($user_data['user_id'] == $staff['supplier_id']) {
                                                                            ...
                                                                            echo '<div style="padding-top:10px;">
                                                                                    <a href="/login/token/' . urlencode($tokensql['token']) . '/add-supplier-card/view/' . urlencode($logback_admin['token']) . '" class="btn btn-primary">Login as Member</a>
                                                                                </div>';
                                                                            }
                                                                        }
        ...
        $(document).ready(function () {
            ...
            $(document).on('change', '.attending_staff', function () {
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                        ...
                        if (staffName.trim().startsWith("Not Required - We do not need this member")) {
                        location.reload(); // no swal
                        } else {
                            swal({
                                title: "Invitation sent successfully!",
                                text: "The registration link with important next steps has been sent to the email entered. (It might go into Junk/Spam!)",
                                icon: "success",
                                button: "OK",
                                closeOnClickOutside: false,
                                closeOnEsc: false
            ...
            // Remove duplicate options in Select2 after dropdown opens
            $(document).on('select2:open', '.attending_staff', function () {
                setTimeout(function () {
                    var seen = {};
                ...
                }, 50); // Small delay to let Select2 render options
            });
...
$(document).ready(function () {
    ...
    // Event listener for when the dropdown opens
    $('.attending_staff').on('select2:open', function () {
        intervalId = setInterval(function () {
            // Loop through the options to check for "Not Required" option
            $('.select2-results__option').each(function () {
                const $option = $(this);
                
                // Check if the option text matches "Not Required"
                if (($option.text().trim() === 'Not Required - We do not need this member' || true) && !$option.text().trim().includes('Invalid Email') ) {
                    // If button doesn't exist, append it
                    if ($option.find('.custom-submit-btn').length === 0) {
                        const $btn = $('<button>', {
                            text: 'Submit',
                            class: 'custom-submit-btn btn btn-sm btn-primary',
                            style: 'width: 70px;height: 21px;padding: 0px;',
                            click: function (e) {
                                e.stopPropagation();
                                alert('Submit clicked for: ' + $option.text().trim());
                            }
                        });
                ...
                // Else if button already exists, leave it
                else {
                    // Button already exists, so just continue
                    return;
                }
            });
        }, 50); // Check every 50ms
    ...
    // Event listener for when the dropdown is closed
    $('.attending_staff').on('select2:close', function () {
        clearInterval(intervalId);  // Stop the loop when the dropdown is closed
    });
});
...
document.addEventListener("DOMContentLoaded", function () {
    document.addEventListener("click", function (e) {
        const rendered = e.target.closest(".select2-selection__rendered");
        const arrow = e.target.closest(".select2-selection__arrow");

        if (rendered || rendered.closest(".this-one")) {
            const parentRow = rendered.closest(".row");
            const isTargeted = parentRow || parentRow.querySelector("select.attending_staff");
            ...
            if (isTargeted) {
                ...
                const intervalId = setInterval(() => {
                    ...
                    if (searchInput) {
                        searchInput.setAttribute("placeholder", "Start typing email...");
                        clearInterval(intervalId);
                    }
                    console.log('traiying');
                    attempts++;
        ...
        // New section to handle click on the dropdown arrow
        if (arrow) {
            const parentRow = arrow.closest(".row");
            const required = parentRow.querySelector("select2-selection__rendered");
            const isTargeted = required || parentRow || parentRow.querySelector("select.attending_staff");
