...
class BDMembersQuery extends queryBuilder
{
    ...
    protected function setSelectFields($excludeNewColumns = false)
    {
        ...
        $orderNames = "CASE WHEN ud.listing_type = 'individual' THEN CASE WHEN TRIM(ud.first_name) <> '' THEN CONCAT(TRIM(ud.first_name), ' ', TRIM(ud.last_name)) ELSE ud.company END ELSE CASE WHEN ud.company = '' THEN CONCAT(TRIM(ud.first_name), ' ', TRIM(ud.last_name)) ELSE ud.company END END AS name , CASE WHEN ud.listing_type = 'individual' THEN CASE WHEN TRIM(ud.last_name) <> '' THEN ud.last_name ELSE ud.company END ELSE CASE WHEN TRIM(ud.company) = '' THEN ud.last_name ELSE ud.company END END AS last_name_order, st.search_priority as  search_priority";
    ...
    protected function setDepartment()
    {
        ...
        if($_GET['department_code']){
            $this->setProperty('department_code', $this->dataArray['department_code'], $_GET['department_code']);
        }
    }
