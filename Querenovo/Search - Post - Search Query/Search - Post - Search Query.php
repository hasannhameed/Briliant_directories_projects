...
$sqlSelectParameters[]  = "st.search_priority";
$sqlSelectParameters[]  = "dp.*";

if( (!isset($recurringEventAddOnInfo['status']) || $recurringEventAddOnInfo['status'] != 'success' || (isset($dc['enable_recurring_events_addon']) && $dc['enable_recurring_events_addon'] == 0)) && ($dc['form_name'] == 'event_fields' || $dc['is_event_feature'] == 1) ){
...
if(!isset($dc['show_expire_post']) || $dc['show_expire_post'] == 0){
    ...
    if ($dc['form_fields_name'] == "blog_article_fields" || $dc['system_name'] == "website_blog_article") {
        ...
        $sqlHavingParameters = array(
            "(expire_date >= '".date('Ymd')."' OR expire_date = '')"
        );
    }
}
...
//if there is a data type and data id on the environment stated, use them for getting the posts from that data category
if ($dc['data_type'] > 0) {
...
if(isset($searchKeyWordAddonInfo['status']) && $searchKeyWordAddonInfo['status'] == 'success' && $_GET['q'] != ""){
    ...
    if($cleanQ !== $queryStringEntitie){
        $flagSpecialChars = true;
    }
    $sqlSearchQ = '      (
        dp.post_title LIKE "%'.$cleanQ.'%" OR 
        dp.post_tags LIKE "%'.$cleanQ.'%" OR 
        dp.post_content LIKE "%'.$cleanQ.'%"';
    if ($flagSpecialChars) {
        $sqlSearchQ .= ' OR dp.post_content LIKE "%' . $queryStringEntitie . '%"';
    }
    $sqlSearchQ .= ' )';
...
if (isset($_GET['daterange']) && $_GET['daterange'] != "") {
    ...
    $finalDateHaving    = $finalDate.date('His');
}else{
...
if( (!isset($dc['show_expire_post']) || $dc['show_expire_post'] == 0) && (!isset($_GET['daterange']) || $_GET['daterange'] == "") ){
    $finalDateHaving = $now;
    if((!isset($recurringEventAddOnInfo['status']) || $recurringEventAddOnInfo['status'] != 'success' || (isset($dc['enable_recurring_events_addon']) && $dc['enable_recurring_events_addon'] == 0)) && ($dc['form_name'] == 'event_fields' || $dc['is_event_feature'] == 1)){
        $sqlHavingParameters[] = "((post_expire_date_having >= '".$finalDateHaving."' OR post_expire_date_having = '') AND start_date > 0)"
        ;
        ...
        $sqlHavingParameters[] = "((expire_date >= '".$finalDateHaving."' OR expire_date >= '".date('Ymd')."' OR expire_date = '') AND start_date > 0)";
    }
}

if((!isset($dc['show_expire_post']) || $dc['show_expire_post'] == 0) && !empty($finalDate) && !empty($finalDate2) && ($dc['form_name'] == 'event_fields' || $dc['is_event_feature'] == 1)){
    ...
    if(isset($recurringEventAddOn['status']) && $recurringEventAddOn['status'] == 'success' && ( $dc['enable_recurring_events_addon'] == 1 || !isset($dc['enable_recurring_events_addon']) ) ){
        ...
        if(isset($_GET['daterange']) && $_GET['daterange'] != ""){
            $sqlWhereParameters[] = "( (post_expire_date >= '".trim($finalDate)."000000' OR post_expire_date = '') AND (post_start_date <= '".trim($finalDate2)."235959') )";
        }else{
            $sqlWhereParameters[] = "(dp.post_start_date <= '".trim($finalDate)."000000' OR (dp.post_start_date <='".trim($finalDate)."000000' AND dp.post_expire_date <= '".trim($finalDate2)."235959') OR dp.post_start_date <= '".trim($finalDate2)."235959')";
        }
        
    }

}else if($dc['show_expire_post'] == 1 && isset($_GET['daterange']) && $_GET['daterange'] != "" && ($dc['form_name'] == 'event_fields' || $dc['is_event_feature'] == 1)){
    $sqlWhereParameters[] = "( ('".trim($finalDate)."000000' <= post_expire_date OR post_expire_date = '') AND ('".trim($finalDate2)."235959' >= post_start_date) )";
}
...
if ($_GET['category'][0] != ""){
    ...
    if(is_array($arr)){
        ...
        $myCategory[] = "'".mysql_real_escape_string(trim($arr))."'";
    }

    if(count($myCategory)){
        $sqlWhereParameters[] = "dp.post_category IN (" . implode(',',$myCategory) . ")";
    }
